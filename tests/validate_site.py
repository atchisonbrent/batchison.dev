...
def main() -> None:
    ...
    effects_js = (ROOT / "assets/js/effects.js").read_text()
    require("initTextPhysics" in main_js, "Shared text physics must be initialized")
    require("initHeroSignal" not in main_js, "The rejected spectral signal must not remain initialized")
    require("text-physics-char" in hero_js, "Text physics must operate on individual glyphs")
    require("TEXT_PHYSICS_FIELDS" in hero_js, "Text physics fields must be explicit and bounded")
    targets_match = re.search(r"TEXT_PHYSICS_FIELDS\s*=\s*\[(.*?)\n\];", hero_js, re.DOTALL)
    if targets_match is None:
        raise AssertionError("Text physics field declaration cannot be parsed")
    physics_targets = targets_match.group(1)
    require('root: ".hero"' in physics_targets and 'root: "#about"' in physics_targets, "Hero and About must each own a bounded physics field")
    for selector in ['.hero-cta .btn', '.private-surfaces-label', '.prose p']:
        require(selector in physics_targets, f"Missing requested text physics target: {selector}")
    require('.private-surfaces a' not in physics_targets, "Private service links must remain outside text physics")
    require("magWords" not in effects_js and ".mag-word" not in css, "About word magnetism must not fight the shared glyph physics")
    ...
    require("const CONTROL_MAX_DISPLACEMENT = 16" in hero_js, "Control-label glyphs must stay visually associated with their fixed hit target")
    require("const STATUS_MAX_DISPLACEMENT = 28" in hero_js, "Private-status glyphs must not obscure adjacent service links")
    require("other.owner !== particle.owner" in hero_js, "Separate text targets must not throw glyphs into each other")
    require('addEventListener("touchstart"' in hero_js, "Text physics must respond immediately to touch")
    require('addEventListener("touchmove"' in hero_js, "Text physics must follow touch movement")
    require('addEventListener("touchend"' in hero_js, "Text physics must settle after touch release")
    require("touches[0]" in hero_js, "Text physics must track the primary touch point")
    ...
    require(".text-physics-char" in css, "Shared text physics glyph styling is missing")
