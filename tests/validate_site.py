...
def main() -> None:
    ...
    require("closestPointOnPointerPath" in hero_js, "Fast cursor movement must use swept contact detection")
    require("pointer.moved" in hero_js, "Pointer impact must be applied once per movement sample")
    require("const POINTER_RADIUS = 7" in hero_js, "Cursor collider must require near-glyph contact")
    require("const DAMPING = 0.72" in hero_js, "Hero physics must use the less-bouncy damping profile")
    require("const IMPACT_MOMENTUM = 0.62" in hero_js, "Cursor velocity must produce a punch impulse")
    require("const COLLISION_RESTITUTION = 0.12" in hero_js, "Glyph collisions must remain deliberately inelastic")
